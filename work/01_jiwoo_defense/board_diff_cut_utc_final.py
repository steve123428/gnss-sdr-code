...
OPTIONAL_DATA = {
    ...
    "SNR (dB-Hz)": ["CN0_SNV_dB_Hz"],
    "code_phase_in_samples": ["aux1"],
    "total_samples": ["aux2"],
    "Doppler Frequency (Hz)": ["carrier_doppler_hz"],
    "Accumulated Carrier Phase (rad)": ["acc_carrier_phase_rad"],
    "rem_code_phase_chips": ["rem_code_phase_chips"],
    "Normalized Code Phase (%)": ["__dummy__"],
}
...
def plot_all(data_pairs, x_pairs=None, x_mode="index", cut_start=1, cut_end=None):
    ...
    fig.suptitle(
        f"Authentic vs Mixed Signal (with Immediate Power Change)",
        # f"Receiver Comparison ({'Elapsed time (sec)' if x_mode=='utc' else 'Index'})  "
        # f"[cut: {cut_label}]",
        fontsize=16
    )

    # 🔑 elapsed seconds → 정수 초로 표시
    def sec_formatter(x, pos):
        return f"{int(x)}"

    formatter = FuncFormatter(sec_formatter)

    for i, (label, (v1, v2, diff)) in enumerate(data_pairs.items()):
        is_bottom = (i == N - 1)

        if x_mode == "utc" and x_pairs and label in x_pairs:
            x = x_pairs[label]   # elapsed seconds (float)
        else:
            x = np.arange(1, len(v1) + 1)

        # ---- main plot ----
        axs[i][0].plot(x, v1, label="Authentic")
        axs[i][0].plot(x, v2, label="Mixed")
...
def compare_files(file1, file2, cut_start=None, cut_end=None, use_utc=False):
    ...
    for label in OPTIONAL_DATA:
        ...
        if use_utc:
            ...
            # ① 먼저 index cut
            v1_cut = v1_full[s0:e0]
            v2_cut = v2_full[s0:e0]
            utc1_cut = utc1[s0:e0]
            utc2_cut = utc2[s0:e0]
            ...
            # ② cut된 데이터에 대해 UTC align
            t, v1, v2 = _align_on_utc(utc1_cut, v1_cut, utc2_cut, v2_cut)
